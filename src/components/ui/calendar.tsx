...
function Calendar({
  ...
  // console.log('bookedDates', new Date(bookedDates[0].from));
  // Combine user's disabled prop with "before today" logic
  const disabledDates = React.useMemo(() => {
    return (date: Date) => {
      return bookedDates.some(
        (bookedDate: { from: string; to: string }) =>
          isWithinInterval(date, {
            start: bookedDate.from,
            end: bookedDate.to,
          }) ||
          isBefore(date, today) ||
          disabled(date)
    ...
    // }
    // // If disabled is not a function, just disable dates before today
    // return (date: Date) => isBefore(date, today);
  }, [disabled, today, bookedDates]);
  ...
  return (
    <DayPicker
      ...
      className={cn(
        'bg-gray-900 text-white group/calendar p-3 [--cell-size:--spacing(8)] [[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent',
      ...
      classNames={{
        ...
        range_start: cn(
          'rounded-l-md bg-yellow-600',
          defaultClassNames.range_start
        ),
        range_middle: cn('rounded-none', defaultClassNames.range_middle),
        range_end: cn(
          'rounded-r-md bg-yellow-600',
